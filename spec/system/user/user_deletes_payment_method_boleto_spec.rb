...
describe 'user deletes payment method boleto' do
    it 'successfully' do
        ...
        expect { click_on 'Apagar' }.to change { BoletoMethod.count }.by(-1)
        
        expect(current_path).to eq(user_company_path(Company.last))
        expect(page).to have_content('boleto apagado com sucesso')
    end
end